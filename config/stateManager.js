...
class DefaultStateManager extends _1.BaseStateManager {
    ...
    /**
     * Adds value to the state trie for the `account`
     * corresponding to `address` at the provided `key` without hashing the key
     * @param address -  Address to set a storage value for
     * @param key - Key to set the value at. Must be 32 bytes long.
     * @param value - Value to set at `key` for account corresponding to `address`. Cannot be more than 32 bytes. Leading zeros are stripped. If it is a empty or filled with zeros, deletes the value.
     */
     async putContractStorageRaw(address, key, value) {
        if (key.length !== 32) {
            throw new Error('Storage key must be 32 bytes long');
        }
        if (value.length > 32) {
        ...
        await this._modifyContractStorage(address, async (storageTrie, done) => {
            if (value && value.length) {
                ...
                await storageTrie.putRaw(key, encodedValue);
            }
            else {
                // deleting a value
                if (this.DEBUG) {
