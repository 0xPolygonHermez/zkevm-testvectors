...
async function main() {
    ...
    const { steps } = data.full_trace;
    const summaryReport = {
        finalCounters: data.txCounters,
    };
    const finalReport = [];
    ...
    // finalReport.sort((a, b) => b.value - a.value);
    ...
    // save summary report
    const pathSummaryReport = path.join(outputBasePath, `${pathInfo.name}-summary.json`);
    fs.writeFileSync(pathSummaryReport, JSON.stringify(summaryReport, null, 2));
}
